...
def my_func(mask):
    ...
    # Detach the input mask only for SciPy operation (to avoid gradients through SciPy)
    bw = mask
    ...
    # Loop to modify `d` based on the condition
    while torch.count_nonzero(d < 0) < 5:
        d -= 1

    # Return the modified tensor, which now has gradients enabled
    return d
...
def get_initial_phi(out_seg):
    binary_seg = torch.round(out_seg)  # Replaces tf.round
    dt_trans = my_func(binary_seg)  # Assuming my_func works with PyTorch tensors
    return dt_trans