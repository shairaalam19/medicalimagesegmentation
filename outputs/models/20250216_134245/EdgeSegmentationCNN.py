...
class EdgeSegmentationCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Intensity Image", type(intensity_image), intensity_image.dtype, intensity_image.shape)
        # Intensity Image <class 'torch.Tensor'> torch.float32 torch.Size([4, 1, 1024, 1024])

        print("ACM Hyperparameters", type(acm_hyperparameters), acm_hyperparameters.dtype, acm_hyperparameters.shape)
        # ACM Hyperparameters <class 'torch.Tensor'> torch.float32 torch.Size([4, 3])

        print("Probability Mask", type(decoded), decoded.dtype, decoded.shape)
        # Intensity Image <class 'torch.Tensor'> torch.float32 torch.Size([4, 1, 1024, 1024])

        sys.exit()
        
        return (acm_hyperparameters, decoded) if self.use_acm else decoded